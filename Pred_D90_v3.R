...
date_chunck <- character() # placeholder char vector to enable importing source('query_body_vars.R') 
...
# date_range <- "SELECT DATEDIFF(CURRENT_DATE(),'2016-10-01')"  # period Josh was using 
date_range <- "SELECT DATEDIFF(CURRENT_DATE(),'2017-07-02')"  # testing shorter period Eric needs 
...
seq(as.Date("2017-08-20"), as.Date("2017-08-20") + 1, by="days")
hist_days <- seq(187,97)

lapply(hist_days, print)


# get list of dates 
# this is used for the install cohort days, to split the data into smaller sizes to run
date_list <- list()
date_list <- seq(as.Date("2017-08-20"), as.Date(format(Sys.Date()-1, format = "%Y-%m-%d")), by="days")
...
for(daily_cohort in seq_along(n_day_chunk_list)) {
  ...
  # n_day_chunk <- paste(dQuote(n_day_chunk[[daily_cohort]]), collapse = ',')
  n_day_chunk <- gsub("[^A-Za-z0-9-\\s]", "', '", n_day_chunk, perl = TRUE)
  n_day_chunk <- paste("'", n_day_chunk, "'")  # add comma of outside of entire string of dates 
  n_day_chunk <- gsub(" ", "", n_day_chunk, fixed = TRUE)  # remove random space that was added in last step
  n_day_chunk_list[[daily_cohort]] <- n_day_chunk
}  

# print(n_day_chunk_list)
# n_day_chunk_list[[1]] #check 
# class(n_day_chunk_list[[1]]) #check 

plus_one <- function(x) x + 1

lapply(date_list, print)
lapply(date_list, class)
lapply(date_list, plus_one)
...
body_syntax <- function(metrics) {
  if(metrics=='Daily_ARPU'){  #if this is the list element name from the levels list 
    body_syntax_vars <- source('query_body_vars.R') # which has the variable conditions
  } else {
    if(metrics=='UA_Daily_Retention'){   
      body_syntax_vars <- source('query_body_vars.R') # which has the variable conditions
    } else {
...
levels_list = list(L1 = c("joindate")
                   ,L2= c("joindate, country, platform")   #available w/ singluar since July 1, 2017 using 
...
for(date_chunck in n_day_chunk) print(date_chunck)
# need to add a level for this 

# query_maker <- function(,  ) {   # need clean up nested loops into a function 
...
for(kpi in metrics) {
  ...
  for(num in seq_along(levels_list)) {  
    ...
    # NEW WAY  
    # for(kpi in n_day_chunk_list) { # create corresponding D1 to Dn incremental periods as
      # query_body <- body_syntax(kpi)  # insert cohort dates into SQL text string from source('query_body.R')
      # query_body <- paste(gsub("[\r\n]", " ", query_body), collapse = " ") # then convert \r carriage returns and \n new lines to spaces & clean-up string space formatting
      # print(query_body)
    # }
...
for(kpi in metrics) print(kpi)

for(date_chunck in n_day_chunk_list[1]) { # create corresponding D1 to Dn incremental periods as
  print(date_chunck)
}     
...
for(num in seq_along(levels_list)) {
  df_check <- as.data.frame(datasets_list[1][[1]][num])  
  df_check[c(1:5),c(1:5)]  %>%  print()        
}
...
rm_if_exists(retention_curve_sums_list) 
retention_curve_sums_list <- list() 
    
for(num in seq_along(levels_list)) {
  groub_by_col_index <- str_count(levels_list[num], ",")  + 2 # count number of commas and add one, this give the number of non data group by columns AND adds a buffer
  # print(groub_by_col_index)  # check
  UA_dataset <- as.data.frame(datasets_list["UA_Daily_Retention"][[1]][num])
  retention_curve_name <- gsub(", ", "_", levels_list[num]) # create name
  print(retention_curve_name)
  df <- UA_dataset[c(1:5),c(1:5)]
  df  %>%  print()  #check
  
  x <- UA_dataset[,c(groub_by_col_index:ncol(UA_dataset))] # captures data only, using the number of non data group by columns and the buffer (made above)
  x[groub_by_col_index:nrow(x),1] <- 0 # overwrites unneeded installs, using the number of non data group by columns and the buffer (made above)
  x %>%  head()  
  # ##### pivots for retention curve
  mat <- as.matrix(x)
  offset <- groub_by_col_index + 1 
  for (position in offset:nrow(mat)) {  # offset used to be a hard coded 3, need to confirm that this should be +1 to the number of grouping columns, seems wrong 
    mat[position,] <- shift(mat[position,],position-2)
  }
  ...
  # 
  # retention_curve_sums_list[[num]] <- retention_curve_sums # append to the list
  # # retention_curve_sums_list[[retention_curve_name]] = retention_curve_sums_list  # name list based on metric
} 

retention_curve_sums_list[1]
retention_curve_sums_list[2]
retention_curve_sums_list[3]
retention_curve_sums_list[4]
